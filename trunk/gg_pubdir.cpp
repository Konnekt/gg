...
unsigned int __stdcall GG::doCntSearch(LPVOID lParam) {
	...
	if (res) {
		...
		LeaveCriticalSection(&GG::searchMap_CS);
	} else {Ctrl->IMessage(&sIMessage_msgBox(IMI_ERROR , "Wyst¹pi³ b³¹d! \nJe¿eli jesteœ po³¹czony z internetem , znaczy ¿e problem le¿y po stronie serwera GG. Spróbuj póŸniej!" , 0 , 0 , src->handle));}
...
CStdString GG::nfoGet(bool noTable , int cnt , int id) {
	...
	/*    if (!noTable) 
	return GETCNTC(cnt , id);
	else
	return UIActionCfgGetValue(sUIAction(
	ICMessage(IMI_ACTION_FINDPARENT , (int)&sUIAction(IMIG_NFO , id|IMIB_CNT))
	,id|IMIB_CNT , cnt) , 0 , 0);
	*/
}
//------------
unsigned int __stdcall GG::doCntDownload(int pMsg) {
	...
	/*    gsr = gg_search_request_mode_3(
	msg->p1?atoi((char*)GETCNTC(msg->p1,CNT_UID))
	:GETINT(CFG_GG_LOGIN)
	, 0, 0);
	*/
	sDIALOG_long sdl;
	...
	if (!res) {
		...
		//ICMessage(IMI_ERROR , (int)"Dane nie zosta³y pobrane z serwera.",0);
