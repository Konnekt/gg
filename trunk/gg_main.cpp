...
void IUpdate(int ver) {
    if (ver < VERSION_TO_NUM(1,0,0,51)) {
        SETSTR(CFG_GG_SERVER , GG_DEF_SERVERS);
    }
}
...
int IPrepare() {
	...
	UIActionAdd(IMIG_GGSTATUS , IMIG_GGSTATUS_SERVER , ACTR_INIT , "" 
		, IDI_SERVER);
	UIActionAdd(IMIG_GGSTATUS , IMIA_GGSTATUS_ONLINE , 0 , "dostêpny"
	, IMessage(IMI_ICONRES,0,0,UIIcon(4,NET_GG,ST_ONLINE,0),IDI_ONLINE));
	UIActionAdd(IMIG_GGSTATUS , IMIA_GGSTATUS_AWAY , 0 , "zaraz wracam"
	, IMessage(IMI_ICONRES,0,0,UIIcon(4,NET_GG,ST_AWAY,0),IDI_AWAY));
	UIActionAdd(IMIG_GGSTATUS , IMIA_GGSTATUS_HIDDEN , 0 , "ukryty"
	, IMessage(IMI_ICONRES,0,0,UIIcon(4,NET_GG,ST_HIDDEN,0),IDI_HIDDEN));
	UIActionAdd(IMIG_GGSTATUS , IMIA_GGSTATUS_OFFLINE , 0 , "niedostêpny"
	, IMessage(IMI_ICONRES,0,0,UIIcon(4,NET_GG,ST_OFFLINE,0),IDI_OFFLINE));


	//  UIActionInsert(IMIG_TRAY , 0 , -1 , ACTT_BARBREAK);
	...
	//  UIActionSetStatus(IMIG_TRAY , IMIA_TRAY_SHUTDOWN , ACTT_BARBREAK , ACTM_TYPE);
	//  IMessage(IMI_UIGROUP,0,0,(int)&UIAfill(IMIG_STATUS , IMIG_GGSTATUS,0,0, UIIcon(4,NET_GG,ST_OFFLINE,0)),(int)"GG");
	...
	UIGroupAdd(IMIG_CFG_USER , IMIG_GGCFG_USER,0,"GG",UIIcon(2,NET_GG,0,0)); {
		UIActionCfgAddPluginInfoBox2(IMIG_GGCFG_USER, 
				...
				//"</div><div class='info'>Do pe³nej obs³ugi wymaganych jest kilka dodatkowych wtyczek (kTransfer, GGImage, Konnferencje)."
				/*HACK: "</div>"*/ 
				, "Wykorzystano bibliotekê <b>LibGadu</b> (http://dev.null.pl/ekg)"
...
/*		UIActionCfgAddPluginInfoBox(IMIG_GGCFG_USER, "Wyorzystano bibliotekê <b>LibGadu</b> (http://dev.null.pl/ekg)"
				"<br/>Strona domowa protoko³u GG - http://www.gadu-gadu.pl/"
				"<br/><br/>Copyright ©2003,2004 <b>Stamina</b>"
				, "res://dll/gglogo.ico", -4);*/
	}
	return 1;
}
 ...
 switch (msg->id) {
  ...
  case IM_MSG_RCV:
            { m = (cMessage*)msg->p1;
              switch (m->type) {
                 case MT_MESSAGE:
                   if (m->flag & MF_SEND) {return IM_MSG_ok;} /*else
                   {
                 ...
                 case MT_QUICKEVENT:
                   ...
                   return IM_MSG_RCV_delete; //quickMessage
                 */  
              }
              return 0;
            }
    case IM_MSG_CHARLIMIT:
            return 2000;

    case IM_MSG_SEND:
            { //if (!ICMessage(IMC_CONNECTED)) return 0;
              cMessage m2;
			  char * errMsg = "Nie mog³em wys³aæ , spróbujê póŸniej";
              m = (cMessage*)msg->p1;
//              sDIALOG_long sdl;
//              sdl.flag = DLONG_BLOCKING | DLONG_NODLG;
...
//                  gg_remove_notify(sess , atoi((char *)GETCNTC(msg->p1,CNT_UID)));
    ...
    case IM_NEW_PROFILE:
        {
			/*
          r = ICMessage(IMI_CONFIRM , (int)"Czy posiadasz ju¿ swoje konto w sieci Gadu-Gadu™?\n" , MB_YESNOCANCEL);
          if (r == IDCANCEL) return 0;
          if (r == IDYES) {
            ICMessage(IMI_INFORM , (int)"Zostaniesz przeniesiony teraz do okna konfiguracji , gdzie bêdziesz móg³ ustawiæ swój UIN ,  has³o i inne ustawienia zwi¹zane z GG");
            ICMessage(IMI_CONFIG , IMIG_GGCFG_USER);
          } else {
            IMessageDirect(IM_UIACTION , 0 , (int)&sUIActionNotify_2params(sUIAction(IMIG_GGCFG_USER , IMIC_GG_ACCOUNT) , ACTN_ACTION , 0 , 0));
          }
		  */
        }
