...
void GG::onUserlistReply(gg_event * e) {
	switch(e->event.userlist.type) {
		case GG_USERLIST_GET_MORE_REPLY:
			if (!e->event.userlist.reply || !*e->event.userlist.reply) break;
...
string GG::getUserList () {
        ...
        for (int i = 1; i < a; i++) {
          ...
          if ((net == NET_GG || net == NET_NONE) && (GETCNTC( i , CNT_DISPLAY)[0] || (net && GETCNTC ( i , CNT_UID))))
            {
				string separator = ";";
				...
				str += (net == NET_GG ? GETCNTC ( i , CNT_UID) : "") + separator;
	...
	for (Stamina::tStringVector::iterator line = lines.begin(); line != lines.end(); ++line) {
		...
		*/
		...
		if (res[fUin].empty()) {
			...
			pos = ICMessage(IMC_CNT_FIND , NET_GG, (int)(res[fUin].c_str()));
		}
		if (pos <= 0) pos = ICMessage(IMC_CNT_ADD , res[fUin].empty()?NET_NONE : NET_GG , (int)res[fUin].c_str());
...
unsigned int __stdcall GG::doListExport(LPVOID lParam) {
	if (onUserListRequest != ulrNone) {ICMessage(IMI_ERROR , (int)"Poprzednia operacja na liœcie kontaktów nie zosta³a zakoñczona!"); return 0;}
