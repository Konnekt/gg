...
unsigned int __stdcall GG::dlgAccount (LPVOID lParam) {
        if (IMessage(IMI_CONFIRM , 0 , 0 , (int)"Zostanie za³o¿one konto na gadu-gadu\nKontynuowaæ ?",MB_TASKMODAL|MB_YESNO)==IDNO) 
			return 0;
        sDIALOG_access sda;
        sda.save = 1;
        sda.title = "GG  - nowe konto [krok 1/3]";
        sda.info = "Podaj has³o do nowego konta";
		if (!IMessage(IMI_DLGSETPASS , 0 , 0 , (int)&sda , 1) || !*sda.pass)  return 0;
		...
		if (!IMessage(IMI_DLGENTER , 0 , 0 , (int)&sde) || !*sde.value)  return 0;
...
unsigned int __stdcall GG::dlgRemoveAccount (LPVOID lParam) {
        ...
        if (!IMessage(IMI_DLGENTER , 0 , 0 , (int)&sde , 1))  return 0;
        string uin = sde.value;

		sDIALOG_access sda;
		sda.title = "GG  - usuwanie konta [krok 2/3]";
        sda.info = "Podaj has³o do konta";
		if (!IMessage(IMI_DLGPASS , 0 , 0 , (int)&sda))  return 0;
        string pass = sda.pass;
...
unsigned int __stdcall GG::dlgNewPass(LPVOID lParam) {
        ...
        if (IMessage(IMI_CONFIRM , 0 , 0 , (int)"Czy na pewno chcesz zmieniæ has³o ?",MB_TASKMODAL|MB_YESNO)==IDNO)  
			return 0;
        sDIALOG_access sda;
        CStdString oldPass;
        sda.save = 0;
		...
		if (!(gghttp = gg_change_passwd4(ggLogin , email.c_str() , oldPass.c_str() , pass.c_str() , tokenid.c_str() , tokenval.c_str() , 0))) {
          ICMessage(IMI_LONGEND , (int)&sdl);
          IMessage(IMI_ERROR , 0 , 0 , (int)stringf("Wyst¹pi³ b³¹d. SprawdŸ po³¹czenie, oraz czy poda³eœ prawid³owy numer konta i has³o i spróbuj ponownie").c_str(),MB_TASKMODAL|MB_OK);
           return 0;
        }
...
unsigned int __stdcall GG::dlgRemindPass(LPVOID lParam) {
        if (!GG::check(1 , 0 , 1 , 1))  return 0;
/*        if (IMessage(IMI_CONFIRM , 0 , 0 , (int)"Has³o zostanie wys³ane na adres e-mail który poda³eœ podczas rejestrowania konta\n Kontynuowaæ?",MB_TASKMODAL|MB_YESNO)==IDNO)
			return 0;
*/
        sDIALOG_enter sde;
		...
		GG::setProxy();
        gg_http * gghttp;
		if (!(gghttp = gg_remind_passwd3
			(GETINT(CFG_GG_LOGIN) , email.c_str(), tokenid.c_str() , tokenval.c_str() , 0))) {
          ICMessage(IMI_LONGEND , (int)&sdl);
...
//            of.nFilterIndex=1;
            ...
            if (GetOpenFileName(&of)) {
               ...
               }  else ICMessage(IMI_ERROR , (int)"Nie mog³em wczytaæ pliku!",MB_TASKMODAL|MB_OK);
            }
            delete [] buff;
            break;}
   ...
   switch (r) {
     case 1:{
            ...
            if (GetSaveFileName(&of)) {
               ...
               } else ICMessage(IMI_ERROR , (int)"Nie mog³em zapisaæ do pliku!",MB_TASKMODAL|MB_OK);
            }
            delete [] buff;
            break;}
